...
- **Error tolerant:** A failed binary does not crash the whole batch. NeoDAX's fault isolation layer handles malformed input gracefully — passes that encounter structural errors emit a recovery notice on stderr and continue. Your script only needs to handle load failures and JS exceptions.
