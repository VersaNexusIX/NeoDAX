...
| Symbols array | `nsymbols x ~420 bytes` |
| Xrefs array | `nxrefs x 24 bytes` |
| Functions array | `nfunctions x ~180 bytes` |
| Blocks array | `nblocks x ~80 bytes` |
| Unicode strings | `nustrings x ~560 bytes` |

For `/bin/ls` (~200 KB): total heap is approximately 3 MB including raw file data.
