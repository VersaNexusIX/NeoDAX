...
As of v1.1.1, NeoDAX applies a three-layer fault isolation model (`include/dax_guard.h`). Each of 32 analysis passes runs independently; a structural error in one pass causes a recovery notice on stderr rather than a crash. Fuzzing is still essential to:
- Find inputs that bypass the isolation layer and cause actual crashes
- Find inputs that trigger the fault path incorrectly on valid binaries
- Measure which passes are most fragile against adversarial data

---

## Quick Robustness Check

Before fuzzing, verify NeoDAX handles obviously malformed input. With v1.1.1+, passes that encounter corrupt data print `[!] pass '...' recovered from fault` on stderr and continue — no crash, no hang.
