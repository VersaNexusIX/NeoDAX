...
LIB_SRCS="src/loader.c src/disasm.c src/x86_decode.c src/arm64_decode.c \
    ...
    src/symexec.c src/decomp.c src/emulate.c src/entropy.c src/dsa.c src/macho.c \
    src/hardening.c src/config.c src/plugin.c"
...
case "$_UNAME_S" in
    Linux)
        case "$_UNAME_M" in
            aarch64) ASM_SRC="arch/arm64_linux.S" ;;
            x86_64)  ASM_SRC="arch/x86_64_linux.S" ;;
        esac ;;
    Darwin)
        case "$_UNAME_M" in
            arm64)  ASM_SRC="arch/arm64_macos.S" ;;
            x86_64) ASM_SRC="arch/x86_64_macos.S" ;;
        esac ;;
    FreeBSD|OpenBSD|NetBSD)
        case "$_UNAME_M" in
            aarch64) ASM_SRC="arch/arm64_bsd.S" ;;
            x86_64)  ASM_SRC="arch/x86_64_bsd.S" ;;
        esac ;;
esac
[ -n "$ASM_SRC" ] && LIB_SRCS="$LIB_SRCS $ASM_SRC"
...
case "$OS" in
    linux)
        LDFLAGS="-Wl,--unresolved-symbols=ignore-all -ldl"
        ;;
    darwin)
...
if is_termux; then
    ...
    # --no-rosegment prevents clang 21 tagged-pointer linker crash on Android
    LDFLAGS="-Wl,--unresolved-symbols=ignore-all -Wl,--no-rosegment -lm -ldl"
    if ! echo "int main(){}" | $CC_BIN -x c - -Wl,--unresolved-symbols=ignore-all -o /dev/null 2>/dev/null; then
        LDFLAGS="-Wl,-z,nodefaultlib -lm -ldl"
    fi
fi
