/* Portable feature-test macros — must come before ALL system headers */
#if defined(__APPLE__)
#  define _DARWIN_C_SOURCE 1
#elif !defined(_GNU_SOURCE)
#  define _GNU_SOURCE 1
#  define _POSIX_C_SOURCE 200809L
#endif

#include <node_api.h>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
#include <stdint.h>
#include <stdbool.h>
#include <ctype.h>
#include <time.h>
#include "dax.h"
#include "dax_guard.h"
#include "x86.h"
#include "arm64.h"
#include "riscv.h"

#define NAPI_CALL(env, call)                                      \
    do {                                                          \
        napi_status _s = (call);                                  \
        if (_s != napi_ok) {                                      \
            const napi_extended_error_info *ei = NULL;            \
            napi_get_last_error_info((env), &ei);                 \
            napi_throw_error((env), NULL,                         \
                ei && ei->error_message ? ei->error_message       \
                                        : "napi call failed");    \
            return NULL;                                          \
        }                                                         \
    } while(0)

static void set_str(napi_env e, napi_value o, const char *k, const char *v) {
    napi_value s; napi_create_string_utf8(e, v?v:"", NAPI_AUTO_LENGTH, &s);
    napi_set_named_property(e, o, k, s);
}
static void set_u32(napi_env e, napi_value o, const char *k, uint32_t v) {
    napi_value n; napi_create_uint32(e, v, &n); napi_set_named_property(e, o, k, n);
}
static void set_i32(napi_env e, napi_value o, const char *k, int32_t v) {
    napi_value n; napi_create_int32(e, v, &n); napi_set_named_property(e, o, k, n);
}
static void set_dbl(napi_env e, napi_value o, const char *k, double v) {
    napi_value n; napi_create_double(e, v, &n); napi_set_named_property(e, o, k, n);
}
static void set_u64(napi_env e, napi_value o, const char *k, uint64_t v) {
    napi_value n; napi_create_bigint_uint64(e, v, &n); napi_set_named_property(e, o, k, n);
}
static void set_bool(napi_env e, napi_value o, const char *k, int v) {
    napi_value b; napi_get_boolean(e, v!=0, &b); napi_set_named_property(e, o, k, b);
}
static void set_null(napi_env e, napi_value o, const char *k) {
    napi_value n; napi_get_null(e, &n); napi_set_named_property(e, o, k, n);
}

static uint64_t get_u64_arg(napi_env env, napi_value v) {
    napi_valuetype vt;
    napi_typeof(env, v, &vt);
    if (vt == napi_bigint) {
        uint64_t out = 0; bool lossless;
        napi_get_value_bigint_uint64(env, v, &out, &lossless);
        return out;
    }
    double d = 0; napi_get_value_double(env, v, &d);
    return (uint64_t)d;
}

static char *get_str_arg(napi_env env, napi_value v, char *buf, size_t bufsz) {
    size_t sz; napi_get_value_string_utf8(env, v, buf, bufsz, &sz);
    return buf;
}

static napi_value build_section(napi_env env, dax_section_t *s) {
    napi_value o; napi_create_object(env, &o);
    set_str(env, o, "name",       s->name);
    set_u64(env, o, "vaddr",      s->vaddr);
    set_u64(env, o, "offset",     s->offset);
    set_u64(env, o, "size",       s->size);
    set_u32(env, o, "flags",      s->flags);
    set_u32(env, o, "insnCount",  s->insn_count);
    const char *t="other";
    switch(s->type){
        case SEC_TYPE_CODE:    t="code";    break;
        case SEC_TYPE_DATA:    t="data";    break;
        case SEC_TYPE_RODATA:  t="rodata";  break;
        case SEC_TYPE_BSS:     t="bss";     break;
        case SEC_TYPE_PLT:     t="plt";     break;
        case SEC_TYPE_GOT:     t="got";     break;
        case SEC_TYPE_DYNAMIC: t="dynamic"; break;
        case SEC_TYPE_DEBUG:   t="debug";   break;
        default: break;
    }
    set_str(env, o, "type", t);
    set_bool(env, o, "isExecutable", (s->flags & 0x4) || (s->flags & 0x20000000));
    set_bool(env, o, "isWritable",   (s->flags & 0x2) != 0);
    set_bool(env, o, "isReadable",   (s->flags & 0x1) != 0);
    return o;
}

static napi_value build_symbol(napi_env env, dax_symbol_t *s) {
    napi_value o; napi_create_object(env, &o);
    set_str(env, o, "name",      s->name);
    set_str(env, o, "demangled", s->demangled[0] ? s->demangled : s->name);
    set_u64(env, o, "address",   s->address);
    set_u64(env, o, "size",      s->size);
    set_bool(env, o, "isEntry",  s->is_entry);
    const char *t="unknown";
    switch(s->type){
        case SYM_FUNC:   t="function"; break;
        case SYM_OBJECT: t="object";   break;
        case SYM_IMPORT: t="import";   break;
        case SYM_EXPORT: t="export";   break;
        case SYM_WEAK:   t="weak";     break;
        case SYM_LOCAL:  t="local";    break;
        default: break;
    }
    set_str(env, o, "type", t);
    return o;
}

static napi_value build_function(napi_env env, dax_func_t *f) {
    napi_value o; napi_create_object(env, &o);
    /* end=0 means the function boundary is unknown (no ret found); use start as fallback */
    uint64_t fn_end  = (f->end > 0) ? f->end : f->start;
    uint64_t fn_size = (fn_end > f->start) ? fn_end - f->start : 0;
    set_str(env, o, "name",       f->name);
    set_u64(env, o, "start",      f->start);
    set_u64(env, o, "end",        fn_end);
    set_u64(env, o, "size",       fn_size);
    set_u32(env, o, "insnCount",  f->insn_count);
    set_u32(env, o, "blockCount", f->block_count);
    set_bool(env, o, "hasLoops",  f->has_loops);
    set_bool(env, o, "hasCalls",  f->has_calls);
    set_i32(env, o, "symIndex",   f->sym_idx);
    return o;
}

static napi_value build_xref(napi_env env, dax_xref_t *x) {
    napi_value o; napi_create_object(env, &o);
    set_u64(env, o, "from",   x->from);
    set_u64(env, o, "to",     x->to);
    set_bool(env, o, "isCall", x->is_call);
    return o;
}

static napi_value build_ustring(napi_env env, dax_ustring_t *u) {
    napi_value o; napi_create_object(env, &o);
    set_u64(env, o, "address",    u->address);
    set_str(env, o, "value",      u->value_utf8);
    set_u32(env, o, "byteLength", u->byte_length);
    const char *enc="ascii";
    switch(u->encoding){
        case STR_ENC_UTF8:    enc="utf-8";    break;
        case STR_ENC_UTF16LE: enc="utf-16le"; break;
        case STR_ENC_UTF16BE: enc="utf-16be"; break;
        default: break;
    }
    set_str(env, o, "encoding", enc);
    return o;
}

static napi_value build_block(napi_env env, dax_block_t *b) {
    napi_value o; napi_create_object(env, &o);
    set_u64(env, o, "start",    b->start);
    set_u64(env, o, "end",      b->end);
    set_i32(env, o, "id",       b->id);
    set_i32(env, o, "funcIdx",  b->func_idx);
    set_bool(env, o, "isEntry", b->is_entry);
    set_bool(env, o, "isExit",  b->is_exit);
    set_i32(env, o, "nsucc",    b->nsucc);
    set_i32(env, o, "npred",    b->npred);
    return o;
}

static napi_value build_info(napi_env env, dax_binary_t *bin) {
    napi_value o; napi_create_object(env, &o);
    if (!bin) return o;
    /* Clamp all counters before serializing — prevents returning crazy values */
    int ns  = DAX_CLAMP(bin->nsections,  0, DAX_MAX_SECTIONS);
    int nsy = DAX_CLAMP(bin->nsymbols,   0, DAX_MAX_SYMBOLS);
    int nfn = DAX_CLAMP(bin->nfunctions, 0, DAX_MAX_FUNCTIONS);
    int nxr = DAX_CLAMP(bin->nxrefs,     0, DAX_MAX_XREFS);
    int nbl = DAX_CLAMP(bin->nblocks,    0, DAX_MAX_BLOCKS);
    int nco = DAX_CLAMP(bin->ncomments,  0, DAX_MAX_COMMENTS);
    int nus = DAX_CLAMP(bin->nustrings,  0, DAX_MAX_USTRINGS);
    set_str(env, o, "file",        bin->filepath[0] ? bin->filepath : "");
    set_str(env, o, "format",      dax_fmt_str(bin->fmt));
    set_str(env, o, "arch",        dax_arch_str(bin->arch));
    set_str(env, o, "os",          dax_os_str(bin->os));
    set_u64(env, o, "entry",       bin->entry);
    set_u64(env, o, "base",        bin->base);
    set_u64(env, o, "imageSize",   bin->image_size);
    set_u64(env, o, "codeSize",    bin->code_size);
    set_u64(env, o, "dataSize",    bin->data_size);
    set_u32(env, o, "totalInsns",  bin->total_insns);
    set_u32(env, o, "nsections",   (uint32_t)ns);
    set_u32(env, o, "nsymbols",    (uint32_t)nsy);
    set_u32(env, o, "nfunctions",  (uint32_t)nfn);
    set_u32(env, o, "nxrefs",      (uint32_t)nxr);
    set_u32(env, o, "nblocks",     (uint32_t)nbl);
    set_u32(env, o, "ncomments",   (uint32_t)nco);
    set_u32(env, o, "nustrings",   (uint32_t)nus);
    set_bool(env, o, "isPie",      bin->is_pie);
    set_bool(env, o, "isStripped", bin->is_stripped);
    set_bool(env, o, "hasDebug",   bin->has_debug);
    set_str(env, o, "sha256",      bin->sha256[0] ? bin->sha256 : "");
    set_str(env, o, "buildId",     bin->build_id[0] ? bin->build_id : "");
    set_str(env, o, "version",     DAX_VERSION);
    return o;
}

static dax_binary_t *get_handle(napi_env env, napi_value obj) {
    if (!env || !obj) return NULL;
    napi_valuetype vt; napi_typeof(env, obj, &vt);
    if (vt != napi_object) return NULL;
    napi_value h;
    if (napi_get_named_property(env, obj, "_handle", &h) != napi_ok) return NULL;
    napi_valuetype ht; napi_typeof(env, h, &ht);
    if (ht != napi_external) return NULL;
    void *p = NULL;
    if (napi_get_value_external(env, h, &p) != napi_ok) return NULL;
    return (dax_binary_t *)p;
}

static void ensure_symbols(dax_binary_t *bin) {
    if (!bin || !bin->data) return;
    if (!bin->symbols) dax_sym_load(bin);
}

static void ensure_xrefs(dax_binary_t *bin) {
    if (!bin || !bin->data) return;
    ensure_symbols(bin);
    if (!bin->xrefs) dax_xref_build(bin);
}

static void ensure_functions(dax_binary_t *bin) {
    int si;
    ensure_symbols(bin);
    if (!bin->functions) {
        for (si = 0; si < bin->nsections && si < DAX_MAX_SECTIONS; si++) {
            dax_section_t *sec = &bin->sections[si];
            if (sec->type != SEC_TYPE_CODE) continue;
            if (sec->size == 0 || sec->offset > bin->size) continue;
            if (sec->size > bin->size - sec->offset) continue;
            dax_func_detect(bin, bin->data + sec->offset,
                            (size_t)sec->size, sec->vaddr, sec);
        }
        dax_clamp_counts(bin);
    }
}


static void ensure_cfg(dax_binary_t *bin) {
    int fi, si;
    ensure_functions(bin);
    if (bin->nblocks == 0 && bin->nfunctions > 0) {
        for (fi = 0; fi < bin->nfunctions && fi < DAX_MAX_FUNCTIONS; fi++) {
            dax_func_t *fn = &bin->functions[fi];
            for (si = 0; si < bin->nsections && si < DAX_MAX_SECTIONS; si++) {
                dax_section_t *sec = &bin->sections[si];
                if (sec->size == 0 || sec->offset > bin->size) continue;
                if (sec->size > bin->size - sec->offset) continue;
                if (fn->start < sec->vaddr || fn->start >= sec->vaddr + sec->size) continue;
                dax_cfg_build(bin, bin->data + sec->offset,
                              (size_t)sec->size, sec->vaddr, fi);
                break;
            }
        }
        dax_clamp_counts(bin);
    }
}

static napi_value ndx_load(napi_env env, napi_callback_info info) {
    size_t argc = 1; napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, NULL, NULL);
    if (argc < 1) { napi_throw_type_error(env, NULL, "load(path)"); return NULL; }

    char path[1024] = {0};
    get_str_arg(env, args[0], path, sizeof(path));

    dax_binary_t *bin = (dax_binary_t *)calloc(1, sizeof(dax_binary_t));
    if (!bin) { napi_throw_error(env, NULL, "OOM"); return NULL; }

    struct timespec t0, t1;
    clock_gettime(CLOCK_MONOTONIC, &t0);

    if (dax_load_binary(path, bin) != 0) {
        free(bin);
        napi_throw_error(env, NULL, "Failed to load binary — check path and permissions");
        return NULL;
    }

    /* Normalize all counters before any downstream code reads them */
    dax_clamp_counts(bin);
    dax_compute_sha256(bin);

    clock_gettime(CLOCK_MONOTONIC, &t1);
    double ms = (t1.tv_sec-t0.tv_sec)*1000.0 + (t1.tv_nsec-t0.tv_nsec)/1e6;

    napi_value ext; napi_create_external(env, bin, NULL, NULL, &ext);
    napi_value result; napi_create_object(env, &result);
    napi_set_named_property(env, result, "info",     build_info(env, bin));
    napi_set_named_property(env, result, "_handle",  ext);
    set_dbl(env, result, "loadTimeMs", ms);
    return result;
}

static napi_value ndx_close(napi_env env, napi_callback_info info) {
    size_t argc = 1; napi_value args[1];
    napi_get_cb_info(env, info, &argc, args, NULL, NULL);
    if (argc < 1) { napi_throw_type_error(env, NULL, "close(handle)"); return NULL; }
    void *p = NULL; napi_get_value_external(env, args[0], &p);
    if (p) { dax_free_binary((dax_binary_t *)p); free(p); }
    napi_value u; napi_get_undefined(env, &u); return u;
}

static napi_value ndx_sections(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    int n = DAX_CLAMP(bin->nsections, 0, DAX_MAX_SECTIONS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_section(env,&bin->sections[i]));
    return arr;
}

static napi_value ndx_symbols(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    ensure_symbols(bin);
    int n = DAX_CLAMP(bin->nsymbols, 0, DAX_MAX_SYMBOLS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_symbol(env,&bin->symbols[i]));
    return arr;
}

static napi_value ndx_functions(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    ensure_functions(bin);
    int n = DAX_CLAMP(bin->nfunctions, 0, DAX_MAX_FUNCTIONS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_function(env,&bin->functions[i]));
    return arr;
}

static napi_value ndx_xrefs(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    ensure_xrefs(bin);
    int n = DAX_CLAMP(bin->nxrefs, 0, DAX_MAX_XREFS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_xref(env,&bin->xrefs[i]));
    return arr;
}

static napi_value ndx_unicode(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    if (!bin->ustrings) dax_scan_unicode(bin);
    int n = DAX_CLAMP(bin->nustrings, 0, DAX_MAX_USTRINGS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_ustring(env,&bin->ustrings[i]));
    return arr;
}

static napi_value ndx_blocks(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    ensure_cfg(bin);
    int n = DAX_CLAMP(bin->nblocks, 0, DAX_MAX_BLOCKS);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for (int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_block(env,&bin->blocks[i]));
    return arr;
}

static napi_value ndx_disasm(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    char section[64] = ".text";
    if (argc>=2) { napi_valuetype vt; napi_typeof(env,args[1],&vt); if(vt==napi_string) get_str_arg(env,args[1],section,sizeof(section)); }
    dax_opts_t opts; memset(&opts,0,sizeof(opts));
    opts.show_addr=1; opts.color=0; opts.symbols=1; opts.strings=1;
    opts.start_addr=0; opts.end_addr=(uint64_t)-1;
    strncpy(opts.section,section,sizeof(opts.section)-1);
    ensure_symbols(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream = open_memstream(&buf,&bsz);
    if (!stream) { napi_throw_error(env,NULL,"open_memstream failed"); return NULL; }
    if      (bin->arch==ARCH_X86_64)  dax_disasm_x86_64(bin,&opts,stream);
    else if (bin->arch==ARCH_ARM64)   dax_disasm_arm64(bin,&opts,stream);
    else if (bin->arch==ARCH_RISCV64) dax_disasm_riscv64(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if (buf) free(buf);
    return r;
}

static napi_value ndx_disasm_json(napi_env env, napi_callback_info info) {
    size_t argc=3; napi_value args[3]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }

    char section[64] = ".text";
    uint32_t limit = 5000;
    uint32_t offset_n = 0;

    if (argc>=2) { napi_valuetype vt; napi_typeof(env,args[1],&vt); if(vt==napi_string) get_str_arg(env,args[1],section,sizeof(section)); }
    if (argc>=3) {
        napi_valuetype vt; napi_typeof(env,args[2],&vt);
        if (vt==napi_object) {
            napi_value v;
            if (napi_get_named_property(env,args[2],"limit",&v)==napi_ok) { napi_valuetype vt2; napi_typeof(env,v,&vt2); if(vt2==napi_number){double d;napi_get_value_double(env,v,&d);limit=(uint32_t)d;}}
            if (napi_get_named_property(env,args[2],"offset",&v)==napi_ok){ napi_valuetype vt2; napi_typeof(env,v,&vt2); if(vt2==napi_number){double d;napi_get_value_double(env,v,&d);offset_n=(uint32_t)d;}}
        }
    }

    dax_section_t *sec = NULL;
    for (int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++) {
        if(!strcmp(bin->sections[i].name,section)) {sec=&bin->sections[i];break;}
    }
    if (!sec || sec->size == 0 || sec->offset > bin->size ||
        sec->size > bin->size - sec->offset) {
        napi_value a; napi_create_array_with_length(env,0,&a); return a;
    }

    ensure_symbols(bin);

    uint8_t *code = bin->data+sec->offset;
    size_t   sz   = (size_t)sec->size;
    uint64_t base = sec->vaddr;
    size_t   off  = 0;
    uint32_t idx  = 0;
    uint32_t count= 0;

    napi_value arr; napi_create_array_with_length(env,0,&arr);

    if (bin->arch==ARCH_X86_64) {
        while (off < sz) {
            x86_insn_t insn;
            memset(&insn,0,sizeof(insn));
            int len = x86_decode(code+off,sz-off,base+off,&insn);
            if (len<=0){off++;continue;}
            if (idx >= offset_n && count < limit) {
                napi_value o; napi_create_object(env,&o);
                set_u64(env,o,"address",insn.address);
                set_str(env,o,"mnemonic",insn.mnemonic);
                set_str(env,o,"operands",insn.ops);
                set_u32(env,o,"size",(uint32_t)insn.length);
                uint8_t ilen = insn.length > 15 ? 15 : insn.length;
                napi_value ba; napi_create_array_with_length(env,(size_t)ilen,&ba);
                for (int b=0;b<(int)ilen;b++){napi_value bv;napi_create_uint32(env,code[off+b],&bv);napi_set_element(env,ba,(uint32_t)b,bv);}
                napi_set_named_property(env,o,"bytes",ba);
                dax_symbol_t *sym = dax_sym_find(bin,insn.address);
                if (sym) set_str(env,o,"symbol",sym->demangled[0]?sym->demangled:sym->name);
                else     set_null(env,o,"symbol");
                const char *grp = dax_igrp_str(dax_classify_x86(insn.mnemonic));
                set_str(env,o,"group",grp&&grp[0]?grp:"unknown");
                napi_set_element(env,arr,count,o);
                count++;
            }
            idx++;
            off+=(size_t)insn.length;
        }
    } else if (bin->arch==ARCH_ARM64) {
        while (off+4<=sz) {
            uint32_t raw=(uint32_t)(code[off])|(code[off+1]<<8)|(code[off+2]<<16)|(code[off+3]<<24);
            a64_insn_t insn; memset(&insn,0,sizeof(insn)); a64_decode(raw,base+off,&insn);
            if (idx>=offset_n && count<limit) {
                napi_value o; napi_create_object(env,&o);
                set_u64(env,o,"address",base+off);
                set_str(env,o,"mnemonic",insn.mnemonic);
                set_str(env,o,"operands",insn.operands);
                set_u32(env,o,"size",4);
                napi_value ba; napi_create_array_with_length(env,4,&ba);
                for(int b=0;b<4;b++){napi_value bv;napi_create_uint32(env,code[off+b],&bv);napi_set_element(env,ba,(uint32_t)b,bv);}
                napi_set_named_property(env,o,"bytes",ba);
                dax_symbol_t *sym=dax_sym_find(bin,base+off);
                if(sym) set_str(env,o,"symbol",sym->demangled[0]?sym->demangled:sym->name);
                else    set_null(env,o,"symbol");
                const char *grp=dax_igrp_str(dax_classify_arm64(insn.mnemonic));
                set_str(env,o,"group",grp&&grp[0]?grp:"unknown");
                napi_set_element(env,arr,count,o); count++;
            }
            idx++; off+=4;
        }
    } else if (bin->arch==ARCH_RISCV64) {
        /* RISC-V RV64GC — instructions are 2 (compressed) or 4 bytes */
        while (off < sz) {
            rv_insn_t insn;
            memset(&insn,0,sizeof(insn));
            int rlen = rv_decode(code+off, sz-off, base+off, &insn);
            if (rlen<=0){off++;continue;}
            if (idx>=offset_n && count<limit) {
                napi_value o; napi_create_object(env,&o);
                set_u64(env,o,"address",insn.address);
                set_str(env,o,"mnemonic",insn.mnemonic);
                set_str(env,o,"operands",insn.operands);
                set_u32(env,o,"size",(uint32_t)rlen);
                uint8_t blen=(uint8_t)(rlen>4?4:rlen);
                napi_value ba; napi_create_array_with_length(env,(size_t)blen,&ba);
                for(int b=0;b<(int)blen;b++){napi_value bv;napi_create_uint32(env,code[off+b],&bv);napi_set_element(env,ba,(uint32_t)b,bv);}
                napi_set_named_property(env,o,"bytes",ba);
                dax_symbol_t *sym=dax_sym_find(bin,insn.address);
                if(sym) set_str(env,o,"symbol",sym->demangled[0]?sym->demangled:sym->name);
                else    set_null(env,o,"symbol");
                const char *grp=dax_igrp_str(dax_classify_riscv(insn.mnemonic));
                set_str(env,o,"group",grp&&grp[0]?grp:"unknown");
                napi_set_element(env,arr,count,o); count++;
            }
            idx++; off+=(size_t)rlen;
        }
    }
    return arr;
}

static napi_value ndx_analyze(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }

    struct timespec t0,t1; clock_gettime(CLOCK_MONOTONIC,&t0);

    ensure_symbols(bin);
    ensure_xrefs(bin);
    ensure_functions(bin);
    ensure_cfg(bin);
    if (!bin->ustrings) dax_scan_unicode(bin);

    clock_gettime(CLOCK_MONOTONIC,&t1);
    double ms=(t1.tv_sec-t0.tv_sec)*1000.0+(t1.tv_nsec-t0.tv_nsec)/1e6;

    napi_value result; napi_create_object(env,&result);

    int nsym2 = DAX_CLAMP(bin->nsymbols,  0, DAX_MAX_SYMBOLS);
    napi_value sym_arr; napi_create_array_with_length(env,(size_t)nsym2,&sym_arr);
    for(int i=0;i<nsym2;i++) napi_set_element(env,sym_arr,(uint32_t)i,build_symbol(env,&bin->symbols[i]));
    napi_set_named_property(env,result,"symbols",sym_arr);

    int nfn2 = DAX_CLAMP(bin->nfunctions, 0, DAX_MAX_FUNCTIONS);
    napi_value fn_arr; napi_create_array_with_length(env,(size_t)nfn2,&fn_arr);
    for(int i=0;i<nfn2;i++) napi_set_element(env,fn_arr,(uint32_t)i,build_function(env,&bin->functions[i]));
    napi_set_named_property(env,result,"functions",fn_arr);

    int nxr2 = DAX_CLAMP(bin->nxrefs, 0, DAX_MAX_XREFS);
    napi_value xr_arr; napi_create_array_with_length(env,(size_t)nxr2,&xr_arr);
    for(int i=0;i<nxr2;i++) napi_set_element(env,xr_arr,(uint32_t)i,build_xref(env,&bin->xrefs[i]));
    napi_set_named_property(env,result,"xrefs",xr_arr);

    int nbl2 = DAX_CLAMP(bin->nblocks,    0, DAX_MAX_BLOCKS);
    napi_value bl_arr; napi_create_array_with_length(env,(size_t)nbl2,&bl_arr);
    for(int i=0;i<nbl2;i++) napi_set_element(env,bl_arr,(uint32_t)i,build_block(env,&bin->blocks[i]));
    napi_set_named_property(env,result,"blocks",bl_arr);

    int nus2 = DAX_CLAMP(bin->nustrings,   0, DAX_MAX_USTRINGS);
    napi_value us_arr; napi_create_array_with_length(env,(size_t)nus2,&us_arr);
    for(int i=0;i<nus2;i++) napi_set_element(env,us_arr,(uint32_t)i,build_ustring(env,&bin->ustrings[i]));
    napi_set_named_property(env,result,"unicodeStrings",us_arr);

    napi_set_named_property(env,result,"info",build_info(env,bin));
    set_dbl(env,result,"analysisTimeMs",ms);
    return result;
}

static napi_value ndx_sym_at(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"sym_at(handle,address)"); return NULL; }
    ensure_symbols(bin);
    uint64_t addr=get_u64_arg(env,args[1]);
    dax_symbol_t *sym=dax_sym_find(bin,addr);
    if (!sym){napi_value n;napi_get_null(env,&n);return n;}
    return build_symbol(env,sym);
}

static napi_value ndx_func_at(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"func_at(handle,address)"); return NULL; }
    ensure_functions(bin);
    uint64_t addr=get_u64_arg(env,args[1]);
    dax_func_t *fn=dax_func_find(bin,addr);
    if (!fn){napi_value n;napi_get_null(env,&n);return n;}
    return build_function(env,fn);
}

static napi_value ndx_xrefs_to(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"xrefs_to(handle,address)"); return NULL; }
    ensure_xrefs(bin);
    uint64_t addr=get_u64_arg(env,args[1]);
    dax_xref_t out[64]; int n=dax_xref_find_to(bin,addr,out,64);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for(int i=0;i<n;i++) napi_set_element(env,arr,(uint32_t)i,build_xref(env,&out[i]));
    return arr;
}

static napi_value ndx_xrefs_from(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"xrefs_from(handle,address)"); return NULL; }
    ensure_xrefs(bin);
    uint64_t addr=get_u64_arg(env,args[1]);
    napi_value arr; napi_create_array_with_length(env,0,&arr);
    uint32_t count=0;
    int nxr = DAX_CLAMP(bin->nxrefs, 0, DAX_MAX_XREFS);
    for(int i=0;i<nxr && count<256;i++)
        if(bin->xrefs[i].from==addr){napi_set_element(env,arr,count++,build_xref(env,&bin->xrefs[i]));}
    return arr;
}

static napi_value ndx_section_by_name(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"section_by_name(handle,name)"); return NULL; }
    char name[64]={0}; get_str_arg(env,args[1],name,sizeof(name));
    for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++)
        if(!strcmp(bin->sections[i].name,name)) return build_section(env,&bin->sections[i]);
    napi_value n; napi_get_null(env,&n); return n;
}

static napi_value ndx_section_at(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<2) { napi_throw_error(env,NULL,"section_at(handle,address)"); return NULL; }
    uint64_t addr=get_u64_arg(env,args[1]);
    for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++){
        dax_section_t *s=&bin->sections[i];
        if(addr>=s->vaddr && addr<s->vaddr+s->size) return build_section(env,s);
    }
    napi_value n; napi_get_null(env,&n); return n;
}

static napi_value ndx_read_bytes(napi_env env, napi_callback_info info) {
    size_t argc=3; napi_value args[3]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin||argc<3) { napi_throw_error(env,NULL,"read_bytes(handle,address,length)"); return NULL; }
    uint64_t addr=get_u64_arg(env,args[1]);
    double dlen=0; napi_get_value_double(env,args[2],&dlen);
    size_t rlen=(size_t)dlen;
    if (rlen==0||rlen>65536){napi_value n;napi_get_null(env,&n);return n;}
    for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++){
        dax_section_t *s=&bin->sections[i];
        if(s->size == 0 || s->offset > bin->size) continue;
        if(s->size > bin->size - s->offset) continue;
        if(addr<s->vaddr||addr>=s->vaddr+s->size) continue;
        uint64_t off=s->offset+(addr-s->vaddr);
        if(off>=bin->size) continue;
        if(rlen > bin->size - (size_t)off) rlen=(size_t)(bin->size-off);
        napi_value buf; void *bdata;
        napi_create_arraybuffer(env,rlen,&bdata,&buf);
        memcpy(bdata,bin->data+off,rlen);
        napi_value view; napi_create_typedarray(env,napi_uint8_array,rlen,buf,0,&view);
        return view;
    }
    /* Fallback: Mach-O and PIE binaries where vaddr-base gives file offset */
    if(bin->nsections>0 && bin->base>0 && addr>=bin->base){
        for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++){
            dax_section_t *s=&bin->sections[i];
            if(s->type!=SEC_TYPE_CODE||s->offset==0) continue;
            if(s->size == 0 || s->offset > bin->size) continue;
            if(s->size > bin->size - s->offset) continue;
            uint64_t vdiff = addr - s->vaddr;
            if(vdiff >= s->size) continue;
            uint64_t off = s->offset + vdiff;
            if(off >= bin->size) continue;
            if(rlen > bin->size - (size_t)off) rlen=(size_t)(bin->size-off);
            napi_value buf; void *bdata;
            napi_create_arraybuffer(env,rlen,&bdata,&buf);
            memcpy(bdata,bin->data+off,rlen);
            napi_value view; napi_create_typedarray(env,napi_uint8_array,rlen,buf,0,&view);
            return view;
        }
    }
    napi_value n; napi_get_null(env,&n); return n;
}

static napi_value ndx_hottest_functions(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    uint32_t topn=20;
    if (argc>=2){double d;napi_get_value_double(env,args[1],&d);topn=(uint32_t)d;}
    ensure_functions(bin); ensure_xrefs(bin);

    int nfn = DAX_CLAMP(bin->nfunctions, 0, DAX_MAX_FUNCTIONS);
    int nxr = DAX_CLAMP(bin->nxrefs,     0, DAX_MAX_XREFS);

    typedef struct { int fi; int calls; } hot_t;
    hot_t *hot=(hot_t*)calloc((size_t)nfn,sizeof(hot_t));
    if (!hot){napi_value n;napi_get_null(env,&n);return n;}
    for(int i=0;i<nfn;i++) hot[i].fi=i;
    for(int x=0;x<nxr;x++){
        if(!bin->xrefs[x].is_call) continue;
        uint64_t to=bin->xrefs[x].to;
        for(int i=0;i<nfn;i++)
            if(bin->functions[i].start==to){hot[i].calls++;break;}
    }
    for(int i=0;i<nfn-1;i++)
        for(int j=i+1;j<nfn;j++)
            if(hot[j].calls>hot[i].calls){hot_t tmp=hot[i];hot[i]=hot[j];hot[j]=tmp;}

    if (topn>(uint32_t)nfn) topn=(uint32_t)nfn;
    napi_value arr; napi_create_array_with_length(env,(size_t)topn,&arr);
    for(uint32_t i=0;i<topn;i++){
        napi_value o; napi_create_object(env,&o);
        napi_value fn=build_function(env,&bin->functions[hot[i].fi]);
        napi_set_named_property(env,o,"function",fn);
        set_i32(env,o,"callCount",hot[i].calls);
        napi_set_element(env,arr,i,o);
    }
    free(hot);
    return arr;
}

static napi_value ndx_strings(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }

    napi_value arr; napi_create_array_with_length(env,0,&arr);
    uint32_t count=0;
    int nsc = DAX_CLAMP(bin->nsections, 0, DAX_MAX_SECTIONS);
    for(int si=0;si<nsc;si++){
        dax_section_t *sec=&bin->sections[si];
        if(sec->type==SEC_TYPE_CODE) continue;
        if(sec->size == 0 || sec->offset > bin->size) continue;
        if(sec->size > bin->size - sec->offset) continue;
        uint8_t *p=(uint8_t*)bin->data+sec->offset;
        size_t slen=(size_t)sec->size;
        for(size_t i=0;i<slen&&count<8192;){
            if(!isprint(p[i])){i++;continue;}
            size_t j=i;
            while(j<slen&&j-i<1024&&isprint(p[j])){j++;}
            if(j-i>=4&&(j>=slen||p[j]==0)){
                napi_value o; napi_create_object(env,&o);
                set_u64(env,o,"address",sec->vaddr+i);
                char tmp[1024+1]; size_t l=j-i<1024?j-i:1024; memcpy(tmp,p+i,l); tmp[l]=0;
                set_str(env,o,"value",tmp);
                set_u32(env,o,"length",(uint32_t)l);
                napi_set_element(env,arr,count++,o);
            }
            i=j+1;
        }
    }
    return arr;
}

static napi_value ndx_version(napi_env env, napi_callback_info info) {
    (void)info; napi_value v; napi_create_string_utf8(env,DAX_VERSION,NAPI_AUTO_LENGTH,&v); return v;
}

static napi_value ndx_info(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin = get_handle(env, args[0]);
    if (!bin) { napi_throw_error(env,NULL,"Invalid handle"); return NULL; }
    return build_info(env,bin);
}


static napi_value ndx_symexec(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    if(func_idx>=0) dax_symexec_func(bin,func_idx,&opts,stream);
    else            dax_symexec_all(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}

static napi_value ndx_ssa(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    ensure_functions(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    if(func_idx>=0) dax_ssa_lift_func(bin,func_idx,&opts,stream);
    else            dax_ssa_lift_all(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}

static napi_value ndx_decompile(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    ensure_functions(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    if(func_idx>=0) dax_decompile_func(bin,func_idx,&opts,stream);
    else            dax_decompile_all(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}

static napi_value ndx_emulate(napi_env env, napi_callback_info info) {
    size_t argc=3; napi_value args[3]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=0;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    uint64_t init_regs[8]={0};
    if(argc>=3){
        napi_valuetype vt; napi_typeof(env,args[2],&vt);
        if(vt==napi_object){
            static const char *rn[]={"x0","x1","x2","x3","x4","x5","x6","x7"};
            for(int i=0;i<8;i++){
                char key[4]; snprintf(key,4,"%d",i);
                napi_value v; napi_valuetype vt2;
                /* positional key "0".."7" */
                if(napi_get_named_property(env,args[2],key,&v)==napi_ok){
                    napi_typeof(env,v,&vt2);
                    if(vt2==napi_bigint){bool l;napi_get_value_bigint_uint64(env,v,&init_regs[i],&l);}
                    else if(vt2==napi_number){double dd;napi_get_value_double(env,v,&dd);init_regs[i]=(uint64_t)dd;}
                }
                /* named key "x0".."x7" */
                if(napi_get_named_property(env,args[2],rn[i],&v)==napi_ok){
                    napi_typeof(env,v,&vt2);
                    if(vt2==napi_bigint){bool l;napi_get_value_bigint_uint64(env,v,&init_regs[i],&l);}
                    else if(vt2==napi_number){double dd;napi_get_value_double(env,v,&dd);init_regs[i]=(uint64_t)dd;}
                }
            }
            /* "sp" → x7 slot */
            {napi_value v; napi_valuetype vt2;
             if(napi_get_named_property(env,args[2],"sp",&v)==napi_ok){
                 napi_typeof(env,v,&vt2);
                 if(vt2==napi_bigint){bool l;napi_get_value_bigint_uint64(env,v,&init_regs[7],&l);}
                 else if(vt2==napi_number){double dd;napi_get_value_double(env,v,&dd);init_regs[7]=(uint64_t)dd;}
             }}
        }
    }
    ensure_functions(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_emulate_func(bin,func_idx,init_regs,8,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}


static napi_value ndx_entropy(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_entropy_scan(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}

static napi_value ndx_rda(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    char section_name[64]=".text";
    if(argc>=2){napi_valuetype vt;napi_typeof(env,args[1],&vt);
        if(vt==napi_string)get_str_arg(env,args[1],section_name,sizeof(section_name));}
    dax_section_t *sec=NULL;
    for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++) {
        dax_section_t *s2 = &bin->sections[i];
        if(strcmp(s2->name,section_name)==0){sec=s2;break;}
    }
    if(!sec){
        /* Section not found — return empty string (not null) so typeof === 'string' */
        napi_value empty; napi_create_string_utf8(env,"",0,&empty); return empty;
    }
    ensure_symbols(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_rda_section(bin,sec,sec->vaddr,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}

static napi_value ndx_ivf(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_ivf_scan(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) { free(buf); }
    return r;
}


/* ── build_block (extended) ─────────────────────────────────────────────────
 * Replaces the earlier build_block — adds isPoly, isSmc, succ[], edgeType[].
 * We redefine the static inline below; the original must be removed.
 * Done via a macro-rename trick: original fn becomes _build_block_old (never
 * called), new one is build_block_v2 registered under the same JS name.
 * ─────────────────────────────────────────────────────────────────────────── */
static napi_value build_block_full(napi_env env, dax_block_t *b) {
    napi_value o; napi_create_object(env, &o);
    set_u64(env, o, "start",    b->start);
    set_u64(env, o, "end",      b->end);
    set_i32(env, o, "id",       b->id);
    set_i32(env, o, "funcIdx",  b->func_idx);
    set_bool(env, o, "isEntry", b->is_entry);
    set_bool(env, o, "isExit",  b->is_exit);
    set_bool(env, o, "isPoly",  b->is_poly);
    set_bool(env, o, "isSmc",   b->is_smc);
    set_i32(env, o, "nsucc",    b->nsucc);
    set_i32(env, o, "npred",    b->npred);
    /* succ[] array */
    napi_value succ_arr; napi_create_array_with_length(env, (size_t)b->nsucc, &succ_arr);
    for (int i = 0; i < b->nsucc; i++) {
        napi_value v; napi_create_int32(env, b->succ[i], &v);
        napi_set_element(env, succ_arr, (uint32_t)i, v);
    }
    napi_set_named_property(env, o, "succ", succ_arr);
    /* edgeType[] array */
    const char *etype_names[] = { "fall","jump","cond_true","cond_false","call","ret","indirect" };
    napi_value et_arr; napi_create_array_with_length(env, (size_t)b->nsucc, &et_arr);
    for (int i = 0; i < b->nsucc; i++) {
        int et = (int)b->edge_type[i];
        const char *ets = (et >= 0 && et < 7) ? etype_names[et] : "unknown";
        napi_value v; napi_create_string_utf8(env, ets, NAPI_AUTO_LENGTH, &v);
        napi_set_element(env, et_arr, (uint32_t)i, v);
    }
    napi_set_named_property(env, o, "edgeType", et_arr);
    return o;
}

/* ── Poly map ──────────────────────────────────────────────────────────────── */
static napi_value ndx_poly(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_poly_map(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* polyJson — structured poly region array */
static napi_value ndx_poly_json(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    /* poly_map populates bin->poly_regions; use open_memstream so it
       doesn't fail on platforms where /dev/null is restricted */
    char *discard=NULL; size_t dsz=0;
    FILE *dev=open_memstream(&discard,&dsz);
    if(dev){dax_poly_map(bin,&opts,dev);fclose(dev);}
    if(discard) free(discard);
    int n=DAX_CLAMP(bin->npoly_regions,0,DAX_POLY_MAX);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for(int i=0;i<n;i++){
        dax_poly_region_t *pr=&bin->poly_regions[i];
        napi_value o; napi_create_object(env,&o);
        set_u64(env,o,"start",         pr->start);
        set_u64(env,o,"end",           pr->end);
        set_u64(env,o,"size",          pr->end>pr->start?pr->end-pr->start:0);
        set_i32(env,o,"mutationScore", pr->mutation_score);
        set_str(env,o,"technique",     pr->technique);
        set_str(env,o,"obfuscator",    pr->obfuscator);
        napi_set_element(env,arr,(uint32_t)i,o);
    }
    return arr;
}

/* ── AIRE ──────────────────────────────────────────────────────────────────── */
static napi_value ndx_aire(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    /* poly_map must run first — use open_memstream so it works everywhere */
    char *poly_discard=NULL; size_t pdsz=0;
    FILE *pdev=open_memstream(&poly_discard,&pdsz);
    if(pdev){dax_poly_map(bin,&opts,pdev);fclose(pdev);}
    if(poly_discard) free(poly_discard);
    /* run AIRE */
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_aire_analyze(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* aireJson — structured insights array */
static napi_value ndx_aire_json(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    /* Step 1: poly_map populates bin->poly_regions */
    char *pd=NULL; size_t pdsz=0;
    FILE *pdev=open_memstream(&pd,&pdsz);
    if(pdev){dax_poly_map(bin,&opts,pdev);fclose(pdev);}
    if(pd) free(pd);
    /* Step 2: aire_analyze populates bin->aire_insights */
    char *ad=NULL; size_t adsz=0;
    FILE *adev=open_memstream(&ad,&adsz);
    if(adev){dax_aire_analyze(bin,&opts,adev);fclose(adev);}
    if(ad) free(ad);
    /* Step 3: serialize */
    int n=DAX_CLAMP(bin->naire_insights,0,DAX_AIRE_MAX);
    napi_value arr; napi_create_array_with_length(env,(size_t)n,&arr);
    for(int i=0;i<n;i++){
        dax_aire_insight_t *ins=&bin->aire_insights[i];
        napi_value o; napi_create_object(env,&o);
        set_u64(env,o,"addr",       ins->addr);
        set_str(env,o,"category",   ins->category);
        set_str(env,o,"insight",    ins->insight);
        set_i32(env,o,"confidence", ins->confidence);
        napi_set_element(env,arr,(uint32_t)i,o);
    }
    return arr;
}

/* ── VM Trace ──────────────────────────────────────────────────────────────── */
static napi_value ndx_vm_trace(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_vm_trace(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* ── CFG (text + JSON) ─────────────────────────────────────────────────────── */
static napi_value ndx_cfg(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    if(func_idx<0) func_idx=0;
    /* build on demand */
    {
        int has_blocks=0;
        for(int bi=0;bi<bin->nblocks;bi++) if(bin->blocks[bi].func_idx==func_idx){has_blocks=1;break;}
        if(!has_blocks && func_idx<bin->nfunctions){
            dax_func_t *fn=&bin->functions[func_idx];
            /* derive end */
            if(fn->end==0||fn->end<=fn->start){
                uint64_t nearest=(uint64_t)-1;
                for(int j=0;j<bin->nfunctions&&j<DAX_MAX_FUNCTIONS;j++){
                    if(j==func_idx) continue;
                    if(bin->functions[j].start>fn->start&&bin->functions[j].start<nearest)
                        nearest=bin->functions[j].start;
                }
                for(int j=0;j<bin->nsymbols&&j<DAX_MAX_SYMBOLS;j++){
                    if(bin->symbols[j].address>fn->start&&bin->symbols[j].address<nearest)
                        nearest=bin->symbols[j].address;
                }
                if(nearest!=(uint64_t)-1) fn->end=nearest;
            }
            for(int si=0;si<bin->nsections&&si<DAX_MAX_SECTIONS;si++){
                dax_section_t *sec=&bin->sections[si];
                if(fn->start>=sec->vaddr && fn->start<sec->vaddr+sec->size &&
                   sec->size<=bin->size-sec->offset && sec->size>0 && sec->offset<=bin->size){
                    dax_cfg_build(bin,bin->data+sec->offset,(size_t)sec->size,sec->vaddr,func_idx);
                    break;
                }
            }
        }
    }
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_cfg_print(bin,func_idx,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

static napi_value ndx_cfg_json(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    if(func_idx<0) func_idx=0;
    /* ensure cfg built */
    {
        int has_blocks=0;
        for(int bi=0;bi<bin->nblocks;bi++) if(bin->blocks[bi].func_idx==func_idx){has_blocks=1;break;}
        if(!has_blocks && func_idx<bin->nfunctions){
            dax_func_t *fn=&bin->functions[func_idx];
            if(fn->end==0||fn->end<=fn->start){
                uint64_t nearest=(uint64_t)-1;
                for(int j=0;j<bin->nfunctions&&j<DAX_MAX_FUNCTIONS;j++){
                    if(j==func_idx) continue;
                    if(bin->functions[j].start>fn->start&&bin->functions[j].start<nearest)
                        nearest=bin->functions[j].start;
                }
                for(int j=0;j<bin->nsymbols&&j<DAX_MAX_SYMBOLS;j++){
                    if(bin->symbols[j].address>fn->start&&bin->symbols[j].address<nearest)
                        nearest=bin->symbols[j].address;
                }
                if(nearest!=(uint64_t)-1) fn->end=nearest;
            }
            for(int si=0;si<bin->nsections&&si<DAX_MAX_SECTIONS;si++){
                dax_section_t *sec=&bin->sections[si];
                if(fn->start>=sec->vaddr && fn->start<sec->vaddr+sec->size &&
                   sec->size<=bin->size-sec->offset && sec->size>0 && sec->offset<=bin->size){
                    dax_cfg_build(bin,bin->data+sec->offset,(size_t)sec->size,sec->vaddr,func_idx);
                    break;
                }
            }
        }
    }
    napi_value arr; napi_create_array_with_length(env,0,&arr);
    uint32_t count=0;
    for(int i=0;i<bin->nblocks;i++){
        if(bin->blocks[i].func_idx==func_idx)
            napi_set_element(env,arr,count++,build_block_full(env,&bin->blocks[i]));
    }
    return arr;
}

/* ── Callgraph ─────────────────────────────────────────────────────────────── */
static napi_value ndx_callgraph(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin); ensure_xrefs(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    dax_callgraph_print(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

static napi_value ndx_callgraph_json(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin); ensure_xrefs(bin);
    /* Build edges from xrefs (calls only) */
    int cg_nfn = DAX_CLAMP(bin->nfunctions, 0, DAX_MAX_FUNCTIONS);
    int cg_nxr = DAX_CLAMP(bin->nxrefs,     0, DAX_MAX_XREFS);
    napi_value nodes; napi_create_array_with_length(env,(size_t)cg_nfn,&nodes);
    for(int i=0;i<cg_nfn;i++){
        napi_value o; napi_create_object(env,&o);
        set_str(env,o,"name",  bin->functions[i].name);
        set_u64(env,o,"start", bin->functions[i].start);
        set_i32(env,o,"idx",   i);
        napi_set_element(env,nodes,(uint32_t)i,o);
    }
    napi_value edges; napi_create_array_with_length(env,0,&edges);
    uint32_t ec=0;
    for(int x=0;x<cg_nxr;x++){
        if(!bin->xrefs[x].is_call) continue;
        int from_fi=-1, to_fi=-1;
        for(int i=0;i<cg_nfn;i++){
            if(bin->xrefs[x].from>=bin->functions[i].start &&
               (bin->functions[i].end==0||bin->xrefs[x].from<bin->functions[i].end))
                from_fi=i;
            if(bin->functions[i].start==bin->xrefs[x].to)
                to_fi=i;
        }
        if(from_fi>=0 && to_fi>=0 && from_fi!=to_fi){
            napi_value e; napi_create_object(env,&e);
            set_i32(env,e,"from", from_fi);
            set_i32(env,e,"to",   to_fi);
            set_u64(env,e,"addr", bin->xrefs[x].from);
            napi_set_element(env,edges,ec++,e);
        }
    }
    napi_value result; napi_create_object(env,&result);
    napi_set_named_property(env,result,"nodes",nodes);
    napi_set_named_property(env,result,"edges",edges);
    return result;
}

/* ── Loops ─────────────────────────────────────────────────────────────────── */
static napi_value ndx_loops(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=-1;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    ensure_cfg(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    if(func_idx>=0) dax_loop_detect(bin,func_idx,stream,0);
    else            dax_loop_print_all(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* ── Switch detect ─────────────────────────────────────────────────────────── */
static napi_value ndx_switch_detect(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    char section_name[64]=".text";
    if(argc>=2){napi_valuetype vt;napi_typeof(env,args[1],&vt);
        if(vt==napi_string) get_str_arg(env,args[1],section_name,sizeof(section_name));}
    dax_section_t *sec=NULL;
    for(int i=0;i<bin->nsections && i<DAX_MAX_SECTIONS;i++)
        if(!strcmp(bin->sections[i].name,section_name)){sec=&bin->sections[i];break;}
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    if(sec && sec->size>0 && sec->offset<=bin->size && sec->size<=bin->size-sec->offset)
        dax_switch_detect(bin,&opts,bin->data+sec->offset,(size_t)sec->size,sec->vaddr,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* ── disasmFunc ────────────────────────────────────────────────────────────── */
static napi_value ndx_disasm_func(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    int func_idx=0;
    if(argc>=2){double d;napi_get_value_double(env,args[1],&d);func_idx=(int)d;}
    ensure_functions(bin);
    if(func_idx<0||func_idx>=bin->nfunctions){
        napi_value empty; napi_create_string_utf8(env,"",0,&empty); return empty;}
    dax_func_t *fn=&bin->functions[func_idx];
    /* derive end */
    if(fn->end==0||fn->end<=fn->start){
        uint64_t nearest=(uint64_t)-1;
        for(int j=0;j<bin->nfunctions&&j<DAX_MAX_FUNCTIONS;j++){
            if(j==func_idx) continue;
            if(bin->functions[j].start>fn->start&&bin->functions[j].start<nearest)
                nearest=bin->functions[j].start;
        }
        for(int j=0;j<bin->nsymbols&&j<DAX_MAX_SYMBOLS;j++){
            if(bin->symbols[j].address>fn->start&&bin->symbols[j].address<nearest)
                nearest=bin->symbols[j].address;
        }
        if(nearest!=(uint64_t)-1) fn->end=nearest;
    }
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0; opts.show_addr=1;
    opts.symbols=1; opts.start_addr=fn->start; opts.end_addr=fn->end;
    if      (bin->arch==ARCH_X86_64)  dax_disasm_x86_64(bin,&opts,stream);
    else if (bin->arch==ARCH_ARM64)   dax_disasm_arm64(bin,&opts,stream);
    else if (bin->arch==ARCH_RISCV64) dax_disasm_riscv64(bin,&opts,stream);
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* ── disasmRange ───────────────────────────────────────────────────────────── */
static napi_value ndx_disasm_range(napi_env env, napi_callback_info info) {
    size_t argc=3; napi_value args[3]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin||argc<2){napi_throw_error(env,NULL,"disasmRange(handle,addr,nInsns)");return NULL;}
    uint64_t addr=get_u64_arg(env,args[1]);
    int n_insns=32;
    if(argc>=3){double d;napi_get_value_double(env,args[2],&d);n_insns=(int)d;}
    ensure_symbols(bin);
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
    if(!stream){napi_throw_error(env,NULL,"open_memstream failed");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0; opts.show_addr=1;
    opts.symbols=1; opts.start_addr=addr; opts.end_addr=(uint64_t)-1;
    /* decode n_insns instructions from addr */
    for(int si=0;si<bin->nsections&&si<DAX_MAX_SECTIONS;si++){
        dax_section_t *sec=&bin->sections[si];
        if(addr<sec->vaddr||addr>=sec->vaddr+sec->size) continue;
        if(sec->size==0||sec->offset>bin->size||sec->size>bin->size-sec->offset) continue;
        uint8_t *code=bin->data+sec->offset;
        size_t   sz=(size_t)sec->size;
        uint64_t base=sec->vaddr;
        size_t   off=(size_t)(addr-base);
        int      count=0;
        if(bin->arch==ARCH_ARM64){
            while(off+4<=sz && count<n_insns){
                uint32_t raw=(uint32_t)(code[off])|(code[off+1]<<8)|(code[off+2]<<16)|(code[off+3]<<24);
                a64_insn_t insn; a64_decode(raw,base+off,&insn);
                dax_symbol_t *sym=dax_sym_find(bin,base+off);
                if(sym) fprintf(stream,"  <%s>:\n",sym->demangled[0]?sym->demangled:sym->name);
                fprintf(stream,"  0x%016llx  %s %s\n",(unsigned long long)(base+off),insn.mnemonic,insn.operands);
                off+=4; count++;
            }
        } else if(bin->arch==ARCH_X86_64){
            while(off<sz && count<n_insns){
                x86_insn_t insn; int l=x86_decode(code+off,sz-off,base+off,&insn);
                if(l<=0) break;
                dax_symbol_t *sym=dax_sym_find(bin,base+off);
                if(sym) fprintf(stream,"  <%s>:\n",sym->demangled[0]?sym->demangled:sym->name);
                fprintf(stream,"  0x%016llx  %s %s\n",(unsigned long long)(base+off),insn.mnemonic,insn.ops);
                off+=(size_t)l; count++;
            }
        } else {
            while(off<sz && count<n_insns){
                rv_insn_t insn; int l=rv_decode(code+off,sz-off,base+off,&insn);
                if(l<=0) break;
                dax_symbol_t *sym=dax_sym_find(bin,base+off);
                if(sym) fprintf(stream,"  <%s>:\n",sym->demangled[0]?sym->demangled:sym->name);
                fprintf(stream,"  0x%016llx  %s %s\n",(unsigned long long)(base+off),insn.mnemonic,insn.operands);
                off+=(size_t)l; count++;
            }
        }
        break;
    }
    fclose(stream);
    napi_value r; napi_create_string_utf8(env,buf?buf:"",bsz,&r);
    if(buf) free(buf);
    return r;
}

/* ── saveDaxc ──────────────────────────────────────────────────────────────── */
static napi_value ndx_save_daxc(napi_env env, napi_callback_info info) {
    size_t argc=2; napi_value args[2]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin||argc<2){napi_throw_error(env,NULL,"saveDaxc(handle,path)");return NULL;}
    char path[1024]={0}; get_str_arg(env,args[1],path,sizeof(path));
    dax_opts_t opts; memset(&opts,0,sizeof(opts));
    int rc=dax_daxc_write(bin,&opts,path);
    napi_value ret; napi_get_boolean(env,rc==0,&ret); return ret;
}

/* ── addComment ────────────────────────────────────────────────────────────── */
static napi_value ndx_add_comment(napi_env env, napi_callback_info info) {
    size_t argc=3; napi_value args[3]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin||argc<3){napi_throw_error(env,NULL,"addComment(handle,addr,text)");return NULL;}
    uint64_t addr=get_u64_arg(env,args[1]);
    char text[512]={0}; get_str_arg(env,args[2],text,sizeof(text));
    dax_comment_add(bin,addr,text);
    napi_value u; napi_get_undefined(env,&u); return u;
}

/* ── obfuscationScore ──────────────────────────────────────────────────────── */
static napi_value ndx_obfuscation_score(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin);
    ensure_xrefs(bin);
    /* Run IVF scan to populate obfuscation score fields */
    {char *d=NULL; size_t dsz=0;
     FILE *dev=open_memstream(&d,&dsz);
     if(dev){dax_opts_t o;memset(&o,0,sizeof(o));dax_ivf_scan(bin,&o,dev);fclose(dev);}
     if(d) free(d);}
    int total = bin->obf_score_smc*3 + bin->obf_score_opaque*2 +
                bin->obf_score_indirect + bin->obf_score_antidebug*2;
    const char *level = total == 0  ? "CLEAN"   :
                        total <= 3  ? "LOW"      :
                        total <= 8  ? "MEDIUM"   :
                        total <= 15 ? "HIGH"     : "EXTREME";
    napi_value o; napi_create_object(env,&o);
    set_i32(env,o,"smc",       bin->obf_score_smc);
    set_i32(env,o,"opaque",    bin->obf_score_opaque);
    set_i32(env,o,"indirect",  bin->obf_score_indirect);
    set_i32(env,o,"antidebug", bin->obf_score_antidebug);
    set_i32(env,o,"total",     total);
    set_str(env,o,"level",     level);
    return o;
}

/* ── vmDetect ──────────────────────────────────────────────────────────────── */
static napi_value ndx_vm_detect(napi_env env, napi_callback_info info) {
    size_t argc=1; napi_value args[1]; napi_get_cb_info(env,info,&argc,args,NULL,NULL);
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    ensure_functions(bin); ensure_xrefs(bin);
    int vd_nfn = DAX_CLAMP(bin->nfunctions,        0, DAX_MAX_FUNCTIONS);
    int vd_nri = DAX_CLAMP(bin->nresolved_indirect, 0, 128);
    napi_value arr; napi_create_array_with_length(env,0,&arr);
    uint32_t count=0;
    for(int fi=0;fi<vd_nfn;fi++){
        int dispatches=0;
        for(int xi=0;xi<vd_nri;xi++){
            uint64_t from=bin->resolved_indirect_from[xi];
            dax_func_t *fn=&bin->functions[fi];
            if(from>=fn->start && (fn->end==0||from<fn->end)) dispatches++;
        }
        if(dispatches < 2) continue;
        napi_value o; napi_create_object(env,&o);
        set_str(env,o,"funcName",   bin->functions[fi].name);
        set_i32(env,o,"score",      dispatches<6?dispatches:6);
        set_u64(env,o,"dispatchPc", bin->functions[fi].start);
        set_u64(env,o,"tableBase",  0ULL);
        napi_value handlers; napi_create_array_with_length(env,0,&handlers);
        uint32_t hcount=0;
        for(int xi=0;xi<vd_nri;xi++){
            uint64_t from=bin->resolved_indirect_from[xi];
            dax_func_t *fn=&bin->functions[fi];
            if(from>=fn->start && (fn->end==0||from<fn->end)){
                napi_value hv; napi_create_bigint_uint64(env,bin->resolved_indirect_to[xi],&hv);
                napi_set_element(env,handlers,hcount++,hv);
            }
        }
        napi_set_named_property(env,o,"handlers",handlers);
        napi_set_element(env,arr,count++,o);
    }
    return arr;
}

NAPI_MODULE_INIT() {
    napi_value fn;
#define REG(name,func) napi_create_function(env,name,NAPI_AUTO_LENGTH,func,NULL,&fn); napi_set_named_property(env,exports,name,fn)
    REG("load",              ndx_load);
    REG("close",             ndx_close);
    REG("info",              ndx_info);
    REG("sections",          ndx_sections);
    REG("symbols",           ndx_symbols);
    REG("functions",         ndx_functions);
    REG("xrefs",             ndx_xrefs);
    REG("xrefsTo",           ndx_xrefs_to);
    REG("xrefsFrom",         ndx_xrefs_from);
    REG("unicodeStrings",    ndx_unicode);
    REG("blocks",            ndx_blocks);
    REG("disasm",            ndx_disasm);
    REG("disasmJson",        ndx_disasm_json);
    REG("disasmFunc",        ndx_disasm_func);
    REG("disasmRange",       ndx_disasm_range);
    REG("analyze",           ndx_analyze);
    REG("symAt",             ndx_sym_at);
    REG("funcAt",            ndx_func_at);
    REG("sectionByName",     ndx_section_by_name);
    REG("sectionAt",         ndx_section_at);
    REG("readBytes",         ndx_read_bytes);
    REG("hottestFunctions",  ndx_hottest_functions);
    REG("strings",           ndx_strings);
    REG("version",           ndx_version);
    REG("entropy",           ndx_entropy);
    REG("rda",               ndx_rda);
    REG("ivf",               ndx_ivf);
    REG("symexec",           ndx_symexec);
    REG("ssa",               ndx_ssa);
    REG("decompile",         ndx_decompile);
    REG("emulate",           ndx_emulate);
    /* ── New in 1.1.1 ── */
    REG("poly",              ndx_poly);
    REG("polyJson",          ndx_poly_json);
    REG("aire",              ndx_aire);
    REG("aireJson",          ndx_aire_json);
    REG("vmTrace",           ndx_vm_trace);
    REG("cfg",               ndx_cfg);
    REG("cfgJson",           ndx_cfg_json);
    REG("callgraph",         ndx_callgraph);
    REG("callgraphJson",     ndx_callgraph_json);
    REG("loops",             ndx_loops);
    REG("switchDetect",      ndx_switch_detect);
    REG("saveDaxc",          ndx_save_daxc);
    REG("addComment",        ndx_add_comment);
    REG("obfuscationScore",  ndx_obfuscation_score);
    REG("vmDetect",          ndx_vm_detect);
#undef REG
    return exports;
}
