...
function withFile(res, filePath, cb) {
    ...
    let bin;
    try { bin = neodax.load(resolved); cb(bin); }
    catch (e) { err(res, 500, e.message || String(e)); }
    finally { if (bin) bin.close(); }
}
...
const server = http.createServer(async (req, res) => {
    ...
    try { await handler(req, res); } catch (e) { err(res, 500, e.message || String(e)); }
});
