...
static napi_value ndx_poly_json(napi_env env, napi_callback_info info) {
    ...
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    /* run poly_map to populate poly_regions */
    FILE *dev=fopen("/dev/null","w"); if(dev){dax_poly_map(bin,&opts,dev);fclose(dev);}
    napi_value arr; napi_create_array_with_length(env,(size_t)bin->npoly_regions,&arr);
    for(int i=0;i<bin->npoly_regions;i++){
...
static napi_value ndx_aire(napi_env env, napi_callback_info info) {
    ...
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    /* poly_map must run first so poly_regions are populated */
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    FILE *dev=fopen("/dev/null","w"); if(dev){dax_poly_map(bin,&opts,dev);fclose(dev);}
    char *buf=NULL; size_t bsz=0;
    FILE *stream=open_memstream(&buf,&bsz);
...
static napi_value ndx_aire_json(napi_env env, napi_callback_info info) {
    ...
    dax_opts_t opts; memset(&opts,0,sizeof(opts)); opts.color=0;
    FILE *dev=fopen("/dev/null","w"); if(dev){dax_poly_map(bin,&opts,dev);fclose(dev);}
    FILE *stream2=fopen("/dev/null","w");
    if(stream2){dax_aire_analyze(bin,&opts,stream2);fclose(stream2);}
    napi_value arr; napi_create_array_with_length(env,(size_t)bin->naire_insights,&arr);
    for(int i=0;i<bin->naire_insights;i++){
...
static napi_value ndx_callgraph_json(napi_env env, napi_callback_info info) {
    ...
    napi_value nodes; napi_create_array_with_length(env,(size_t)bin->nfunctions,&nodes);
    for(int i=0;i<bin->nfunctions;i++){
        napi_value o; napi_create_object(env,&o);
...
static napi_value ndx_obfuscation_score(napi_env env, napi_callback_info info) {
    ...
    dax_binary_t *bin=get_handle(env,args[0]);
    if(!bin){napi_throw_error(env,NULL,"Invalid handle");return NULL;}
    /* Ensure IVF has run to populate obf scores */
...
static napi_value ndx_vm_detect(napi_env env, napi_callback_info info) {
    ...
    uint32_t count=0;
    for(int fi=0;fi<bin->nfunctions;fi++){
        /* Count resolved indirect dispatches in this function */
        int dispatches=0;
        for(int xi=0;xi<bin->nresolved_indirect;xi++){
        ...
        set_u64(env,o,"tableBase",  0ULL); /* static analysis can't always determine */
        /* handlers: resolved targets in this function */
