...
SSA lifting, the decompiler, reading pseudo-C output, and the ARM64 and RISC-V emulator.
...
- ARM64 and RISC-V (not x86-64 or RISC-V)
...
SSA lifting in NeoDAX is designed for ARM64 and RISC-V. It handles straightforward functions well but may struggle with:
...
`
  },
  {
    ...
    level_label: "5 - Symbolic Execution and Decompilation",
    content: `# Concrete Emulator (ARM64 + RISC-V)

**Level:** 5 - Symbolic Execution and Decompilation
**Prerequisites:** 53_reading_pseudo_c.md
**What You Will Learn:** How the NeoDAX ARM64 and RISC-V emulator works and when concrete emulation is useful.
...
NeoDAX includes a concrete ARM64 and RISC-V emulator that:
...
**What You Will Learn:** Practical ways to apply the ARM64 and RISC-V emulator in real analysis scenarios.
...
`
  },
  {
    ...
    learn: "How .daxc snapshots work — AOT C source format, create, compile, run standalone, and load back into NeoDAX.",
    level_label: "6 - Real World Reverse Engineering",
...
`
  },
  {
    ...
    learn: "Full REST API reference — all endpoints, curl/Python/JS examples, batch scanning, error handling.",
...
| \`POST /api/ssa\` | \`file\`, \`funcIdx\` | SSA IR for function |
...
- Advanced: Decompiler, SSA Form, Sym Exec, Emulator
...
**SSA Form** - lifts a function to Static Single Assignment IR. Select a function and click Lift to SSA.
...
3. Write an IVF plugin that flags any function with more than 20 indirect branches.
4. Use \`-eox-list\` to verify your plugin loaded correctly.

## Next

You have completed Level 7. Continue to \`80_common_patterns_reference.md\` to begin the Appendix.
`
  },
  {
...
**DAXC:** NeoDAX's AOT C source snapshot format. Stores analysis results - functions, disassembly, comments - as a compilable C file (\`.daxc\`). Can be loaded by NeoDAX, compiled standalone with GCC/Clang, or diffed with standard text tools.
...
**Snapshot:** In NeoDAX, a saved analysis result stored as a \`.daxc\` AOT C source file. Magic \`NEOX\`, version 4. Loadable by NeoDAX or compilable directly: \`clang -O2 -o snap file.daxc && ./snap\`.
