    ...
    for (i = 0; i < eh->e_shnum; i++) {
        ...
        if (s->sh_type == 2 || s->sh_type == 11) {
            ...
            if (strlink && strlink < (uint32_t)eh->e_shnum) {
                ...
                uint64_t strsz   = sh[strlink].sh_size;
                if (stroff < bin->size && strsz <= bin->size - stroff) {
            ...
            for (j = 1; j < nent; j++) {
                ...
                if ((uint64_t)sym->st_name >= strtab_size) continue;
...
static int load_elf32_symbols(dax_binary_t *bin) {
    ...
    for (i = 0; i < eh->e_shnum; i++) {
        ...
        if (s->sh_type == 2 || s->sh_type == 11) {
            ...
            if (strlink && strlink < (uint32_t)eh->e_shnum) {
                uint32_t stroff = sh[strlink].sh_offset;
                uint32_t strsz  = sh[strlink].sh_size;
                if (stroff < bin->size &&
                    strsz  <= bin->size - stroff) {
                    strtab      = (char *)(bin->data + stroff);
                    strtab_size = strsz;
                }
            }
            ...
            for (j = 1; j < nent; j++) {
                ...
                if (!strtab || sym->st_name == 0) continue;
                if ((uint32_t)sym->st_name >= strtab_size) continue;
                name = strtab + sym->st_name;
                if (!name[0]) continue;
...
int dax_sym_load(dax_binary_t *bin) {
    ...
    if (bin->nsymbols > 1) {
        qsort(bin->symbols, (size_t)bin->nsymbols, sizeof(dax_symbol_t), sym_cmp);
    }
    return bin->nsymbols;
}
...
dax_symbol_t *dax_sym_find(dax_binary_t *bin, uint64_t addr) {
    int lo, hi;
    if (!bin || !bin->symbols || bin->nsymbols <= 0) return NULL;
    lo = 0; hi = bin->nsymbols - 1;
    while (lo <= hi) {
        int mid = lo + (hi - lo) / 2;   /* overflow-safe midpoint */
        if (mid < 0 || mid >= bin->nsymbols) break;
