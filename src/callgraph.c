...
static int addr_to_func(dax_binary_t *bin, uint64_t addr) {
    int i;
    if (!bin || !bin->functions || bin->nfunctions <= 0) return -1;
...
static cg_t *cg_build(dax_binary_t *bin) {
    ...
    if (!bin || !bin->xrefs || bin->nxrefs < 0) return NULL;
...
static void cg_print_node(dax_binary_t *bin, cg_t *cg,
    ...
    {
        int sym_ok = bin->functions[fi].sym_idx >= 0 &&
...
void dax_callgraph_print(dax_binary_t *bin, dax_opts_t *opts, FILE *out) {
    ...
    if (bin->nfunctions <= 0) { cg_free(cg); return; }
    is_callee = (int *)calloc((size_t)bin->nfunctions, sizeof(int));
    if (!is_callee) { cg_free(cg); return; }

    for (i = 0; i < cg->nedges; i++)
        if (cg->edges[i].callee >= 0 && cg->edges[i].callee < bin->nfunctions && cg->edges[i].callee < DAX_MAX_FUNCTIONS)
