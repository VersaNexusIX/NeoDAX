...
static int collect_loop_nodes(dax_binary_t *bin, int header_idx,
    ...
    *nn = 0;
    ...
    while (sp > 0) {
        int cur = stack[--sp];
        dax_block_t *blk = (cur >= 0 && cur < bin->nblocks && cur < DAX_MAX_BLOCKS) ? &bin->blocks[cur] : NULL;
        int ci = -1;
        if (!blk) continue;
...
int dax_loop_detect(dax_binary_t *bin, int func_idx, FILE *out, int color) {
    ...
    DAX_GUARD_BIN_RET(bin, 0);
    if (!dax_func_idx_ok(bin, func_idx)) return 0;
                ...
                if (nn > 1) {
                    ...
                    for (m = 0; m < nn && m < 8; m++) {
                        ...
                        fprintf(out, "%sblock_%d%s  %s0x%llx%s",
                                ...
                                C_CMT,
                                (unsigned long long)
                                (loop_nodes[m] < bin->nblocks && loop_nodes[m] < DAX_MAX_BLOCKS
