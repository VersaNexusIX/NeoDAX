...
#define MODRM_REG(bits) \
    do { \
        ...
        if (lock && mnem[0] != 'l') {
            ...
            snprintf(mnem, sizeof(mnem), "%s", tmp_m);
        }
        if (repne && mnem[0] != 'r') {
        ...
        snprintf(insn->ops,      sizeof(insn->ops),      "%s", ops);
    }
...
void x86_format(x86_insn_t *insn, char *mnem_out, char *ops_out) {
    snprintf(mnem_out, DAX_MAX_MNEMONIC, "%s", insn->mnemonic);
    snprintf(ops_out,  DAX_MAX_OPERANDS, "%s", insn->ops);
}
