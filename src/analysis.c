...
int dax_xref_build(dax_binary_t *bin) {
    ...
    for (i = 0; i < bin->nsections; i++) {
        ...
        if (bin->arch == ARCH_X86_64) {
            ...
            while (off < code_size) {
                rv_insn_t insn2;
                int rlen = rv_decode(code + off, code_size - off, addr + off, &insn2);
                if (rlen <= 0) { off++; continue; }
                {
                    dax_igrp_t grp = dax_classify_riscv(insn2.mnemonic);
                    if ((grp == IGRP_CALL || grp == IGRP_BRANCH) &&
                        insn2.operands[0] == '0') {
                        uint64_t target = 0;
                        sscanf(insn2.operands, "0x%llx", (unsigned long long *)&target);
...
int dax_xref_find_to(dax_binary_t *bin, uint64_t addr, dax_xref_t *out, int max) {
    int i, found = 0;
    if (!bin || !bin->xrefs || !out || max <= 0) return 0;
    if (bin->nxrefs < 0 || bin->nxrefs > DAX_MAX_XREFS) return 0;
...
int dax_func_detect(dax_binary_t *bin, uint8_t *code, size_t code_size,
    ...
    if (bin->arch == ARCH_X86_64) {
        ...
        while (off < code_size) {
            ...
            if (sym && (sym->type==SYM_FUNC||sym->type==SYM_EXPORT||
                ...
                if (bin->nfunctions < DAX_MAX_FUNCTIONS) {
                    ...
                    snprintf(fn->name, DAX_SYM_NAME_LEN, "%.511s",
                ...
                { int fi2, fnd=0;
                  for (fi2=0;fi2<bin->nfunctions;fi2++)
                      if (bin->functions[fi2].start==cur_addr) {
                          ...
                          snprintf(bin->functions[fi2].name, DAX_SYM_NAME_LEN, "%.511s", sn);
                          fnd=1; break;
                      }
        ...
        while (off < code_size) {
            uint64_t cur_addr = base_addr + off;
            rv_insn_t insn;
            int rlen = rv_decode(code + off, code_size - off, cur_addr, &insn);
            if (rlen <= 0) { off++; continue; }
            ...
            if (sym && (sym->type==SYM_FUNC||sym->type==SYM_EXPORT||sym->is_entry)) {
                    ...
                    bin->functions[bin->nfunctions-1].end = cur_addr;
                {
                    ...
                    for (fi2 = 0; fi2 < bin->nfunctions && fi2 < DAX_MAX_FUNCTIONS; fi2++)
                        if (bin->functions[fi2].start == cur_addr) {
                            bin->functions[fi2].sym_idx = (int)(sym - bin->symbols);
                            snprintf(bin->functions[fi2].name, DAX_SYM_NAME_LEN,
                                     "%.511s",
                                     sym->demangled[0]?sym->demangled:sym->name);
                            fnd = 1; break;
                        }
            ...
            if (!in_func) {
                int is_pro = (!strcmp(insn.mnemonic,"addi") &&
                              strstr(insn.operands,"sp, sp, -")) ||
                             (!strcmp(insn.mnemonic,"c.addi16sp")) ||
                             (cur_addr == base_addr);
                if (!is_pro) {
                    dax_igrp_t grp = dax_classify_riscv(insn.mnemonic);
                    if (grp == IGRP_CALL && insn.operands[0] == '0') {
                        uint64_t tgt = 0;
                        sscanf(insn.operands, "0x%llx", (unsigned long long *)&tgt);
                        if (tgt >= base_addr && tgt < base_addr + code_size) {
                            int dup = 0, fi2;
                            for (fi2 = 0; fi2 < bin->nfunctions && fi2 < DAX_MAX_FUNCTIONS; fi2++)
                                if (bin->functions[fi2].start == tgt) { dup = 1; break; }
                ...
                } else if (bin->nfunctions < DAX_MAX_FUNCTIONS) {
                    dax_func_t *fn = &bin->functions[bin->nfunctions++];
                    fn->start = cur_addr; fn->end = 0; fn->sym_idx = -1;
                    snprintf(fn->name, DAX_SYM_NAME_LEN, "sub_%llx",
                             (unsigned long long)cur_addr);
            ...
            if (in_func && (!strcmp(insn.mnemonic,"ret") ||
                            (!strcmp(insn.mnemonic,"jalr") &&
                             insn.operands[0]=='z' && insn.operands[1]=='e'))) {
                if (bin->nfunctions > 0)
                    bin->functions[bin->nfunctions-1].end = cur_addr + (uint64_t)rlen;
                in_func = 0;
            }
            off += (size_t)rlen;
        }
...
dax_func_t *dax_func_find(dax_binary_t *bin, uint64_t addr) {
    int i;
    if (!bin || !bin->functions || bin->nfunctions <= 0) return NULL;
...
void dax_switch_detect(dax_binary_t *bin, dax_opts_t *opts,
    ...
    if (bin->arch == ARCH_ARM64) {
        ...
        while (off + 4 <= sz) {
            ...
            if (!strcmp(m,"adrp")) {
                ...
                if (sscanf(ops,"%15[^,], 0x%llx",r,(unsigned long long*)&pg)==2) {
                    int ri=(r[0]=='x'||r[0]=='w')?atoi(r+1):-1;
                    if (ri>=0&&ri<32){adrp_val[ri]=pg;reg_adrp[ri]=1;add_val[ri]=0;}
                }
            }
            
            if (!strcmp(m,"add")) {
                char r1[16]="",r2[16]="",imm[32]="";
                if (sscanf(ops,"%15[^,], %15[^,], %31s",r1,r2,imm)==3) {
                    int d=(r1[0]=='x'||r1[0]=='w')?atoi(r1+1):-1;
                    int s=(r2[0]=='x'||r2[0]=='w')?atoi(r2+1):-1;
                    if (d>=0&&d<32&&s>=0&&s<32&&reg_adrp[s]>=1&&imm[0]=='#') {
                        uint64_t ov=(uint64_t)strtoull(imm+1,NULL,0);
                        add_val[d]=adrp_val[s]+ov; reg_adrp[d]=2;
                    }
                }
            }
            ...
            if (!strcmp(m,"cmp")) {
                ...
                if (comma) last_cmp_bound=(int)strtol(comma+1,NULL,0);
            }

            
            if (!strcmp(m,"ldr") && strstr(ops,"uxtx") && off+4<=sz) {
                uint32_t nr=(uint32_t)code[off+4]|(code[off+5]<<8)|
                            (code[off+6]<<16)|(code[off+7]<<24);
                a64_insn_t ni; a64_decode(nr,addr+4,&ni);
                if (!strcmp(ni.mnemonic,"br") && ni.operands[0]=='x') {
                    ...
                    uint64_t tbl_base=(bri>=0&&bri<32&&reg_adrp[bri]>=2)?add_val[bri]:0;

                    int n_cases = (last_cmp_bound>0&&last_cmp_bound<256)?
                                   last_cmp_bound+1 : 0;

                    nfound++;
                    fprintf(out,"\n  %sdispatch #%d%s  %s0x%llx%s",
                            C_SW,nfound,C_RST,C_ADR,
                            (unsigned long long)addr,C_RST);
                    if (n_cases>0)
                        fprintf(out,"  %s%d cases%s",C_CMT,n_cases,C_RST);
                    fprintf(out,"\n");
                    fprintf(out,"  %s│%s  ldr  %s%s%s\n",
                            C_CMT,C_RST,C_ADR,ops,C_RST);
                    fprintf(out,"  %s│%s  br   %s%s%s\n",
                            C_CMT,C_RST,C_ADR,ni.operands,C_RST);

                    if (tbl_base>0) {
                        fprintf(out,"  %s│%s  table base: %s0x%llx%s\n",
                                C_CMT,C_RST,C_ADR,
                                (unsigned long long)tbl_base,C_RST);
                        
                        int ei, max_e=(n_cases>0&&n_cases<=32)?n_cases:8;
                        for (ei=0;ei<max_e;ei++) {
                            uint64_t eaddr=tbl_base+(uint64_t)(ei*8);
                            int si2;
                            for (si2=0;si2<bin->nsections;si2++) {
                                ...
                                dax_symbol_t *sym=dax_sym_find(bin,v);
                                dax_func_t   *fn2=dax_func_find(bin,v);
                                const char *lbl=sym?(sym->demangled[0]?sym->demangled:sym->name):
                                               (fn2?fn2->name:NULL);
                                fprintf(out,"  %s│%s  [%d] %s0x%llx%s",
                                        C_CMT,C_RST,ei,C_GN,
                                        (unsigned long long)v,C_RST);
                                if (lbl&&lbl[0]) fprintf(out," <%s>",lbl);
                                fprintf(out,"\n");
                                break;
                            }
                        }
                    }
                    fprintf(out,"  %s└─%s  computed goto via jump table\n",C_CMT,C_RST);
                    last_cmp_bound=-1; 
                }
            }
            ...
            if (!strcmp(m,"br")||!strcmp(m,"blr")) {
                memset(reg_adrp,0,sizeof(reg_adrp));
            }

            off+=4;
        }
        if (nfound==0)
            fprintf(out,"%s  (no dispatch patterns detected)%s\n",C_CMT,C_RST);
        return;
    }
    ...
    while (off < sz) {
        ...
        snprintf(dummy_mnem, sizeof(dummy_mnem), "%s", cmp_insn.mnemonic);
        snprintf(dummy_ops,  sizeof(dummy_ops),  "%.127s", cmp_insn.ops);
    ...
    if (nfound == 0 && bin->arch != ARCH_RISCV64) {
    ...
    if (bin->arch == ARCH_RISCV64) {
        ...
        while (off < sz) {
            ...
            if (!strcmp(m,"addi")||!strcmp(m,"sltu")||!strcmp(m,"sltiu")) {
                const char *comma = strchr(ops, ',');
                if (comma) { comma = strchr(comma+1, ','); }
                if (comma) last_cmp_bound = (int)strtol(comma+1, NULL, 0);
            }

            if ((!strcmp(m,"jalr")||!strcmp(m,"c.jr")) && ops[0]!='r'&&ops[0]!='z') {
                nfound++;
                fprintf(out, "\n  %sswitch #%d%s  %s0x%llx%s",
                        C_SW, nfound, C_RST, C_ADR,
                        (unsigned long long)(base+off), C_RST);
                if (last_cmp_bound > 0 && last_cmp_bound < 512)
                    fprintf(out, "  %s~%d cases%s", C_CMT, last_cmp_bound+1, C_RST);
                fprintf(out, "\n");
                fprintf(out, "  %s|%s  %s   %s%s%s\n",
                        C_CMT, C_RST, m, C_ADR, ops, C_RST);
                fprintf(out, "  %s|%s  computed goto via jump register\n", C_CMT, C_RST);
                last_cmp_bound = -1;
            }
