...
int a64_decode(uint32_t raw, uint64_t addr, a64_insn_t *insn) {
    ...
    if ((raw & 0xFF800000) == 0x12800000 || (raw & 0xFF800000) == 0x52800000 ||
        (raw & 0xFF800000) == 0x72800000 ||
        (raw & 0xFF800000) == 0x92800000 || (raw & 0xFF800000) == 0xD2800000 ||
        (raw & 0xFF800000) == 0xF2800000) {
        sf    = BIT(raw,31);
        opc   = BITS(raw,30,29);
    ...
    /* SIMD/FP load/store register (unscaled/post-index/pre-index/unsigned offset)
     * Encoding: bits[31:30]=size, bit[26]=1(V), bits[29:27]=1x1/1x0 */
    if ((raw & 0x3A000000U) == 0x38000000U || BIT(raw,26)) {
     ...
     * Encoding: bits[31:30]=opc, bit[26]=1(V), bits[29:27]=0x1x */
    if ((raw & 0x3A000000U) == 0x28000000U && BIT(raw,26)) {
        int V = BIT(raw,26);
        int L = BIT(raw,22);
     ...
     * Encoding: bits[31]=0, bit[29]=op, bits[28:24]=0F, bit[10]=1 */
    if ((raw & 0x1F800400U) == 0x0F000400U) {
    ...
    snprintf(insn->mnemonic, sizeof(insn->mnemonic), "%s", mnem);
    snprintf(insn->operands, sizeof(insn->operands), "%s", ops);
    return 4;
}
