...
static void elf64_extract_build_id(dax_binary_t *bin) {
    int i;
    /* Ensure build_id is always NUL-terminated even if no note section found */
    bin->build_id[0] = '\0';
    for (i = 0; i < bin->nsections; i++) {
        dax_section_t *s = &bin->sections[i];
        if (strcmp(s->name, ".note.gnu.build-id") == 0 ||
            ...
            if (s->offset + s->size <= bin->size && s->size >= 16) {
                ...
                /* Max descsz 30 bytes -> 60 hex chars + NUL fits in build_id[64] */
                if (off2 + descsz <= s->size && descsz <= 30) {
                    uint32_t j;
                    for (j = 0; j < descsz && j < 30; j++)
...
int dax_load_binary(const char *path, dax_binary_t *bin) {
    ...
    dax_harden_filesize(bin->size, 1);

    {
        ...
        if (magic32 == 0x464C457F) {
            dax_harden_elf_header(bin->data, bin->size, 1);
            return dax_parse_elf(bin);
        } else if (magic16 == PE_DOS_MAGIC) {
            dax_harden_pe_header(bin->data, bin->size, 1);
...
int dax_parse_elf(dax_binary_t *bin) {
    ...
    if (!bin || !bin->data || bin->size < 16) return -1;
    ...
    if (eclass == ELFCLASS64) {
        ...
        if (eh->e_shstrndx != 0 && eh->e_shstrndx < eh->e_shnum) {
            ...
            /* overflow-safe offset+size check */
            if (strs->sh_size > 0 &&
                strs->sh_offset < bin->size &&
                strs->sh_size <= bin->size - strs->sh_offset)
        ...
        for (i = 0; i < eh->e_shnum && bin->nsections < DAX_MAX_SECTIONS; i++) {
            ...
            if (shstrtab && s->sh_name &&
                s->sh_name < sh[eh->e_shstrndx].sh_size)
        ...
        if (bin->os == DAX_PLAT_LINUX || bin->os == DAX_PLAT_UNIX) {
            int j;
            for (j = 0; j < bin->nsections && j < DAX_MAX_SECTIONS; j++) {
